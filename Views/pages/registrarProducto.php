...
<body>
  ...
  <main class="main-content container mt-5">
    ...
    <section id="seccion-registrar" class="seccion-formulario">
      ...
      <form method="post" action="/Cliente-Servidor-Farmacia/Controllers/registrarProductoController.php"
    ...
    <section id="seccion-eliminar" class="seccion-formulario">
      ...
      <form method="post" action="/Cliente-Servidor-Farmacia/Views/pages/eliminarProducto.php" class="mx-auto"
    ...
    <section id="seccion-modificar" class="seccion-formulario">
      ...
      <form id="formCargarProducto" method="post"
        action="/Cliente-Servidor-Farmacia/Views/pages/registrarProducto.php#seccion-modificar" class="mb-3">
        ...
        <select id="cmbProducto" name="cmbProducto" class="form-select" required>
          ...
          <?php while ($prod = mysqli_fetch_assoc($productosActivos)): ?>
            <option value="<?= htmlspecialchars($prod["CODIGO"]) ?>" <?= isset($productoModificar["CODIGO"]) && $productoModificar["CODIGO"] === $prod["CODIGO"] ? "selected" : "" ?>>
      ...
      <form method="post" action="/Cliente-Servidor-Farmacia/Controllers/modificarProductoController.php"
        class="mx-auto" style="max-width:600px;" enctype="multipart/form-data">
        <input type="hidden" name="cmbProducto" value="<?= htmlspecialchars($productoModificar['CODIGO'] ?? '') ?>">
        <div class="mb-3">
          <label for="txtNombreNuevo" class="form-label">Nuevo Nombre</label>
          <input type="text" id="txtNombreNuevo" name="txtNombreNuevo"
            value="<?= htmlspecialchars($productoModificar['NOMBRE'] ?? '') ?>" class="form-control" required>
        </div>

        <div class="mb-3">
          <label for="txtPrecioNuevo" class="form-label">Nuevo Precio Unitario</label>
          <input type="number" id="txtPrecioNuevo" name="txtPrecioNuevo" step="50"
            value="<?= htmlspecialchars($productoModificar['PRECIO_UNITARIO'] ?? '') ?>" class="form-control" required>
        </div>
        ...
        <div class="mb-3">
          ...
          <select id="cmbCategoriaNueva" name="cmbCategoriaNueva" class="form-select" required>
            ...
            <?php while ($cat = mysqli_fetch_assoc($categorias)): ?>
              <option value="<?= $cat["ID_CATEGORIA_PRODUCTO"] ?>" <?= isset($productoModificar['ID_CATEGORIA_PRODUCTO']) && (int) $productoModificar['ID_CATEGORIA_PRODUCTO'] === (int) $cat['ID_CATEGORIA_PRODUCTO'] ? 'selected' : '' ?>>
        ...
        <div class="mb-3">
          ...
          <select id="cmbUnidadNueva" name="cmbUnidadNueva" class="form-select" required>
            ...
            <?php while ($uni = mysqli_fetch_assoc($unidades)): ?>
              <option value="<?= $uni["ID_UNIDAD_MEDIDA"] ?>" <?= isset($productoModificar['ID_UNIDAD_MEDIDA']) && (int) $productoModificar['ID_UNIDAD_MEDIDA'] === (int) $uni['ID_UNIDAD_MEDIDA'] ? 'selected' : '' ?>>
